...
mod101q
```
The same log-likelihood - 61 quadrature points is optimal value
...
items_removed
```
**All the removed items come from the 'E' category**

Remove these items from responses:
...
```{r eval=F}
results_SE2 <- mirtCAT(mo = mo, method = "MAP", criteria = "MI", start_item = "MI", local_pattern = responses, cl = cl, design = list(min_SEM = 0.2))
```
...
```{r}
responses_r <- responses[which(responses_demo$months >= 12), ]
nrow(responses_r)
```

```{r eval=F}
results_SE2_r <- mirtCAT(mo = mo, method = "MAP", criteria = "MI", start_item = "MI", local_pattern = responses_r, cl = cl, design = list(min_SEM = 0.2))
```

```{r}
get_sim_results(results_SE2_r, fscores[which(responses_demo$months >= 12), ], params)
```

```{r fig.width=12, fig.height=8}
#Prepare cuts
tests_lengths <- laply(results_SE2_r, function(x) length(x$items_answered))
cuts <- cut(tests_lengths, breaks = c(0, 5, 10, 15, 25, 35, 45, 54, 55), labels = c("1 - 5", "6 - 10", "11 - 15", "16 - 25", "26 - 35", "36 - 45", "46-54", "55 (all)"))
...
```{r}
get_first_item_responses(results_SE2, responses)
```
### Start theta by age and gender

```{r eval=F}
results_SE2_theta <- mirtCAT(mo = mo, method = "MAP", criteria = "MI", start_item = "MI", local_pattern = responses, cl = cl, design = list(min_SEM = 0.2, thetas.start = start_thetas))
```

```{r}
get_sim_results(results_SE2_theta, fscores, params)
```

```{r}
get_first_item_responses(results_SE2_theta, responses)
```

### Decrease starting thetas by 2

```{r eval=F}
results_SE2_theta2 <- mirtCAT(mo = mo, method = "MAP", criteria = "MI", start_item = "MI", local_pattern = responses, cl = cl, design = list(min_SEM = 0.2, thetas.start = start_thetas - 2))
```

```{r}
get_sim_results(results_SE2_theta2, fscores, params)
```

```{r}
get_first_item_responses(results_SE2_theta2, responses)
```