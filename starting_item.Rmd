...
* Starting thetas & simulations
  + First item selections
...
Make simulation with  start thetas by decreased 0.5:
...
Make simulation withstart thetas decreased  by 2:
...
Prepare function obtaining 1st item responses:
...
Obtain percent of selections of the first item (2 means selection):
...
Make simulation with start thetas given by age only:

```{r eval=F}
results_thetasAge<- mirtCAT(mo = mo, method = "MAP", criteria = "MI", start_item = "MI", local_pattern = responses, cl = makeCluster(detectCores()), design = list(min_SEM = 0.1, max_items = 50, min_items = 10, thetas.start = start_thetas2))
```
