...
* Early-stopping CAT simulations with needed SE = 0.15
...
```{r eval=F}
results_SE15 <- data.frame(avglength = NA, cor = NA, avgSE = NA, rel = NA, unused = NA)
  
#Make simulation
cl <- makeCluster(detectCores())
sim_results <- mirtCAT(mo = mo, method = "MAP", criteria = "MI", start_item = "MI", local_pattern = responses, cl = cl, design = list(min_SEM = 0.15))
...
#Update data frame with results
results_SE15 <- rbind(results_SE15, c(avglength, cor, avgSE, rel, unused))

#Prepare cuts
cuts <- cut(tests_lengths, breaks = c(0, 5, 10, 15, 25, 35, 50, 75, 100, 200, 669, 670), labels = c("1 - 5", "6 - 10", "11 - 15", "16 - 25", "26 - 35", "36 - 50", "51 - 75", "76 - 100", "101 - 200", "201 - 669", "670"), dig.lab = 0)

#Plot distribution of tests lengths
plot_SE15 <- ggplot(data.frame(table(cuts)), aes(x = cuts, y = Freq)) +
  xlab("Number of items in the test") +
  ylab("Number of tests") +
  geom_bar(stat = "identity") +
  ggtitle("Tests lengths for SE = 0.15 stop criterion") +
  geom_text(aes(label = Freq), vjust = -0.3) + 
  theme_pubclean() 
  
results_SE15 <- na.omit(results_SE15)
beep(sound=8)
```
